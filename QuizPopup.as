package {
	...
	public class QuizPopup extends MovieClip {
		...
		private function waitForLoad(event: Event): void {
			if (questionAPI.loadDone == "true") { //waits for the data to be grabbed
			...
			} else if (questionAPI.loadDone == "failed") {
				var loadFailDisplay: QuizFailedToLoad = new QuizFailedToLoad();
				loadFailDisplay.x = Main.stg.stageWidth / 2;
				loadFailDisplay.y = Main.stg.stageHeight / 2;
				Main.instance.menuContainer.addChild(loadFailDisplay);
				removeEventListener(Event.ENTER_FRAME, waitForLoad); //removes the event listener as this function is no longer needed
